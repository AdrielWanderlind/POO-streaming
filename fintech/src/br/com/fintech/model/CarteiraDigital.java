...
public class CarteiraDigital {
    ...
    public CarteiraDigital(String codigoCarteira, int capacidadeMaxima) {
         if(codigoCarteira==null || codigoCarteira.trim().isEmpty()) {
            throw new IllegalArgumentException("Codigo da carteira não pode ser nulo ou vazio.");
        }
    ...
    public void adicionarTransacao(Transacao transacao) throws LimiteTransacoesExcedidoException {
        Objects.requireNonNull(transacao, "A transacao nao pode ser nula.");
        if( this.transacoes.size() >= this.capacidadeMaxima){
            throw new LimiteTransacoesExcedidoException(String.format(
                "Carteira '%s' atingiu o limite maximo de %d transacoes.",
                this.codigoCarteira, this.capacidadeMaxima));
        }
